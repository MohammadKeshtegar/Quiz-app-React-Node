  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex justify-between items-center">
        <p className="dark:text-neutral-400 text-xl">users: {chat.members.length}</p>
        {!isInGroup && <Button styleType="fill">Join</Button>}
      </div>
      <div className="w-[350px]">
        <ul className="dark:bg-neutral-700 bg-neutral-200 rounded p-1 max-h-96 overflow-y-auto divide-y-2 divide-neutral-500">
          {chat.members.map((member) => (
            <li key={member._id} className="flex items-center gap-5 px-4 py-3 group transition-all">
              <div className="flex items-center gap-5 w-full">
                ...
                <p className="text-xl font-semibold flex items-center justify-between w-full">
                  ...
                  {member._id === chat.admin && <span className="text-lg dark:text-neutral-400">admin</span>}
                </p>
                {isAdmin && (
                  <>
